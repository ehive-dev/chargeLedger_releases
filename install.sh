...
# chargeLedger Installer/Updater (DietPi / arm64)
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/ehive-dev/chargeLedger_releases/main/install.sh | sudo bash -s -- [--pre|--stable] [--tag vX.Y.Z] [--repo owner/repo]
#   sudo bash install.sh [--pre|--stable] [--tag vX.Y.Z] [--repo owner/repo]

# Produktname vs. technische IDs:
# - Produktname: chargeLedger
# - Debian-Paket / Binary / Service / Pfade: chargeledger
PRODUCT_NAME="chargeLedger"
APP_NAME="chargeledger"
UNIT="${APP_NAME}.service"
UNIT_BASE="${APP_NAME}"

# Optional:
#   REPO=ehive-dev/chargeLedger_releases
#   DPKG_PKG=chargeledger
#   PORT=3020
#   HEALTH_PATH=/healthz

REPO="${REPO:-ehive-dev/chargeLedger_releases}"
...
detect_exec(){
  if [[ -x "/usr/local/bin/${APP_NAME}" ]]; then
    ...
    command -v "${APP_NAME}"
  elif command -v chargeLedger >/dev/null 2>&1; then
    command -v chargeLedger
...
TMPDIR="$(mktemp -d -t chargeledger-install.XXXXX)"
